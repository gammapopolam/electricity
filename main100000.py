...
#offset, buffer, angle = config(200000)
offset = 500
buffer = 100
angle = 10
print(offset)
...
epsg=32635
gdf=importer('samples/truba_utm2.geojson', epsg=epsg)
# Разбиение сети на сегменты
start_time = time.time()
gdf_exploded=explode_gpd(gdf)
print("Explode finished --- %s seconds ---" % (time.time() - start_time))
#print(gdf_exploded)
exporter(gdf_exploded.copy(), name='coursework/truba/200000/exploded.gpkg', keep_debug=True, epsg=epsg)
...
#print(gdf_buffer)
exporter(gdf_buffer.copy(), name='coursework/truba/200000/clipped.gpkg', keep_debug=True, buffer=False, epsg=epsg)
exporter(gdf_buffer.copy(), name='coursework/truba/200000/buffer.gpkg', keep_debug=True, buffer=True, epsg=epsg)
#%%
...
exporter(gdf_processed.copy(), name='coursework/truba/200000/processed.gpkg', keep_debug=True, buffer=False, epsg=epsg)
#%%
...
exporter(gdf_flipped.copy(), name='coursework/truba/200000/flipped.gpkg', keep_debug=True, epsg=epsg)
# Параллельный сдвиг
start_time = time.time()
...
exporter(gdf_unpacked.copy(), name='coursework/truba/200000/offsetted.gpkg', keep_debug=True, epsg=epsg)
#%%
gdf_unpacked_2=unpack_multilines(gdf_offset)
# Восстановление направления сегментов нити
start_time = time.time()
gdf_origin_2=gpd.read_file('coursework/truba/200000/exploded.gpkg')
...
exporter(gdf_recover.copy(), name='coursework/truba/200000/line_recovered.gpkg', keep_debug=True, epsg=32635)
#%%
# Восстановление сети
start_time = time.time()
gdf_net=recover_net(gdf_recover, 'coursework/truba/200000/logs.txt')
print("Recovering finished --- %s seconds ---" % (time.time() - start_time))
exporter(gdf_net.copy(), name='coursework/truba/200000/final.gpkg', keep_debug=True, epsg=32635)
